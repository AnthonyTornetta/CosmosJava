...
public class SimplexNoise { // Simplex noise in 2D, 3D and 4D
	...
	// Old permutation table:
//	private static short p[] = { 151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225, 140, 36, 103, 30, 69,
...
//			180 };
	...
	private short permutationTable[] = new short[256];
	
	// To remove the need for index wrapping, double the permutation table length
	private short permTable[] = new short[512];
	private short permTableMod12[] = new short[512];
	...
	public SimplexNoise(long seed)
	{
		...
		for (int i = 0; i < 512; i++)
		{
			permutationTable[i & 255] = (short)(rdm.nextInt(254) + 1);
			permTable[i] = permutationTable[i & 255];
			permTableMod12[i] = (short) (permTable[i] % 12);
		}
	}
	...
	public double noise(double xin, double yin) {
		...
		int ii = i & 255;
		int jj = j & 255;
		int gi0 = permTableMod12[ii + permTable[jj]];
		int gi1 = permTableMod12[ii + i1 + permTable[jj + j1]];
		int gi2 = permTableMod12[ii + 1 + permTable[jj + 1]];
	...
	public double noise(double xin, double yin, double zin) {
		...
		int jj = j & 255;
		int kk = k & 255;
		int gi0 = permTableMod12[ii + permTable[jj + permTable[kk]]];
		int gi1 = permTableMod12[ii + i1 + permTable[jj + j1 + permTable[kk + k1]]];
		int gi2 = permTableMod12[ii + i2 + permTable[jj + j2 + permTable[kk + k2]]];
		int gi3 = permTableMod12[ii + 1 + permTable[jj + 1 + permTable[kk + 1]]];
	...
	public double noise(double x, double y, double z, double w) {
		...
		int kk = k & 255;
		int ll = l & 255;
		int gi0 = permTable[ii + permTable[jj + permTable[kk + permTable[ll]]]] % 32;
		int gi1 = permTable[ii + i1 + permTable[jj + j1 + permTable[kk + k1 + permTable[ll + l1]]]] % 32;
		int gi2 = permTable[ii + i2 + permTable[jj + j2 + permTable[kk + k2 + permTable[ll + l2]]]] % 32;
		int gi3 = permTable[ii + i3 + permTable[jj + j3 + permTable[kk + k3 + permTable[ll + l3]]]] % 32;
		int gi4 = permTable[ii + 1 + permTable[jj + 1 + permTable[kk + 1 + permTable[ll + 1]]]] % 32;
